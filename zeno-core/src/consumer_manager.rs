...
impl<'a, 's> ConsumerManager<'a, 's> where 'a: 's {
    pub fn new(mmap: &'a [u8]) -> Self {
        ConsumerManager{mmap: mmap, consumers: HashMap::new(), subject_registry: HashMap::new()}
    }
    ...
    pub fn register_new_subject(&mut self, subject: &[u8]) {
        ...
        self.subject_registry.insert(str_subject.clone(), subject.to_vec().into_boxed_slice());
